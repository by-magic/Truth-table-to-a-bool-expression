...
namespace Generators
{
    ...
    class Parser
    {
        ...
        public Tuple<bool, List<Tuple<int, int>>> CreateBracketsList(string expr)
        {
        ...
        private Tuple<int, List<string>> SplitLogicExpression(string expr)
        {
            bool f = true;
            int l = 0;
                ...
                foreach (string op in operations)
                {
                    ...
                    while (index != -1)
                    {
                        ...
                        if (!InBrackets(brackets.Item2, index))
                        {
                            ...
                            {
                                case "not":
                                    lst.Add(expr.Substring(index + op.Length));
                                    break;
                                case "input":
                                    lst.Add(expr);
                                    break;
                                default:
                                    lst.Add(expr.Substring(0, index));
                                    lst.Add(expr.Substring(index + op.Length));
                                    break;
                            }
